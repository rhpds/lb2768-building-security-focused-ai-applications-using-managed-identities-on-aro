...
For the purposes of this training session, we have provisioned a single {ocp} cluster, with {rhoai} deployed on it. In addition we have set up Azure OpenAI GPT-4 and Azure AI Search that we will connect to via their respective APIs. 
...
** https://rhods-dashboard-redhat-ods-applications.{openshift_cluster_ingress_domain}/[https://rhods-dashboard-redhat-ods-applications.{openshift_cluster_ingress_domain}/projects,window=_blank]

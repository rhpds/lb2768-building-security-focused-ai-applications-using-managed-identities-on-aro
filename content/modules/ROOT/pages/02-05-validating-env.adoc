...
. In Launcher click on Terminal:
+
[.bordershadow]
image::02/terminal.png[]
+
.. Run the following command (but replace the url)  to download the credentials to connect to various Azure AI services.

[.lines_space]
[.console-input]
[source, text]
cd
wget https://website.hosting/.env
oc create secret generic dot-env --from-file /opt/app-root/src/.env

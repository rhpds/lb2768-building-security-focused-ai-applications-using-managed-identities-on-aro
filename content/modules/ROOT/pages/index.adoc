...
In this {ic-lab} we are going to use https://azure.microsoft.com/en-us/blog/introducing-gpt4-in-azure-openai-service/[Azure OpenAI GPT-4,window=_blank]. GPT-4 is a large language model that we will use to summarize and extract information from fictive insurance claims. We are then also going to use https://azure.microsoft.com/en-us/products/ai-services/ai-search/[Azure AI Search,window=_blank], it is a vector database that can be used to store documents for retrieval when implementing RAG (Retrieval Augmented Generation).
